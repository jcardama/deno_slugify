...
export function slugify(
  string: string,
  options: { replacement?: string; remove?: string; lower?: boolean } | string =
    {},
): string {
  let _options: { replacement?: string; remove?: string; lower?: boolean } =
    typeof options === "string" ? { replacement: options } : options || {};

  let slug = string
    .split("")
    .reduce((result: string, ch: string): string => (
      result +
      (slugify.charMap[ch] || ch)
        // allowed
...
export namespace slugify {
  ...
  export let extend = function (customMap: { [key: string]: string }): void {
